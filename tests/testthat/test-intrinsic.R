...
test_that("Figure 10 JSPI", {
  ...
  loss <- sapply(phi0, function(phi0) intrinsic_phi0(phi0=phi0, x=6, y=15, S=1, T=1, a = 0.5, b = 0, c = 0.5, d = 0, subdivisions=100)) 
  expect_true(loss[3] < log(10) & loss[4] > log(10))
  expect_true(loss[1] > log(10) & loss[2] < log(10))
  expect_equal(intrinsic_estimate(x=6, y=15, S=1, T=1, a = 0.5, b = 0, c = 0.5, d = 0, subdivisions=100), .4090702, tolerance=1e-7, check.attributes=FALSE)
})
...
test_that("Figure 15 Master", {
  ...
  expect_equal(estimate, 0.505509, tolerance=1e-6, check.attributes=FALSE)
  expect_equal(attr(estimate, "loss"), 0.4693625, tolerance=1e-6)
